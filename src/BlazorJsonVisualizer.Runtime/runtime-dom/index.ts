...
class DomRuntimeControllerImpl implements DomRuntimeController {
  ...
  public async getSchemaMetadataForPath(
  ...
  private render(sessionId: string): void {
    ...
    hostElement.replaceChildren(
      createRuntimeView(
        ...
        {
          selectProjectionItem: (projectionId, selection) =>
            this.selectProjectionItem({ sessionId, projectionId, selection }),
  ...
  for (const row of projection.rows) {
    ...
    for (const cell of row.cells) {
      ...
      const isSelectedCell =
        selection?.kind === "cell" && selection.rowId === row.rowId && selection.columnId === cell.columnId;
      if (isSelectedCell) {
