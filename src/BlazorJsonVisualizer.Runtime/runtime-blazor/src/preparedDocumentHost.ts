...
import {
  ...
  type PreparedDocumentRuntimeClient
} from "./preparedDocumentRuntimeClient.js";
...
export class PreparedDocumentHost {
  ...
  private createRowElement(session: PreparedBrowserSession, row: PreparedRenderRowDto): HTMLElement {
    ...
    rowElement.className =
      row.nodeId === session.focusedNodeId ? "bjv-prepared-row bjv-prepared-row-focused" : "bjv-prepared-row";
  ...
  private looksLikePrimitiveRow(row: PreparedRenderRowDto): boolean {
    ...
    return (
      /:\s*(".*"|-?\d|true\b|false\b|null\b)/u.test(trimmed) || /^(".*"|-?\d|true\b|false\b|null\b)/u.test(trimmed)
    );
  }
  ...
  private createMetadataPanel(metadata: PreparedDocumentMetadataDto): HTMLElement {
