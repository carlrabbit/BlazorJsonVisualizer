...
function isPluginTokensRecord(value: unknown): value is Record<string, ThemePluginTokens> {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return false;
  }
...
export function parseTheme(json: string): ThemeParseResult {
  ...
  } else if (!(SUPPORTED_SCHEMA_VERSIONS as readonly string[]).includes(obj["schemaVersion"])) {
    diagnostics.push({
      field: "schemaVersion",
  ...
  } else if (!(SUPPORTED_MODES as readonly string[]).includes(obj["mode"])) {
    diagnostics.push({
      field: "mode",
  ...
  if (!("tokens" in obj)) {
    ...
    diagnostics.push({
      field: "tokens",
      message: "Field 'tokens' must be an object where all values are strings."
    });
  }
  ...
  if ("plugins" in obj && !isPluginTokensRecord(obj["plugins"])) {
    diagnostics.push({
      field: "plugins",
      message: "Field 'plugins' must be an object where each entry has a 'tokens' map with string values."
    });
  }
  ...
  const theme: ThemeDocument = {
    ...
    plugins: "plugins" in obj ? (obj["plugins"] as Record<string, ThemePluginTokens>) : {}
  };
