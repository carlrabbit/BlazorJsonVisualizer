...
export function tokenize(text: string): JsonToken[] {
  ...
  while (pos < text.length) {
    ...
    if (ch === "{") { tokens.push({ kind: "braceOpen", startOffset: pos, endOffset: pos + 1 }); pos++; continue; }
    if (ch === "}") { tokens.push({ kind: "braceClose", startOffset: pos, endOffset: pos + 1 }); pos++; continue; }
    if (ch === "[") { tokens.push({ kind: "bracketOpen", startOffset: pos, endOffset: pos + 1 }); pos++; continue; }
    if (ch === "]") { tokens.push({ kind: "bracketClose", startOffset: pos, endOffset: pos + 1 }); pos++; continue; }
    if (ch === ":") { tokens.push({ kind: "colon", startOffset: pos, endOffset: pos + 1 }); pos++; continue; }
    if (ch === ",") { tokens.push({ kind: "comma", startOffset: pos, endOffset: pos + 1 }); pos++; continue; }
